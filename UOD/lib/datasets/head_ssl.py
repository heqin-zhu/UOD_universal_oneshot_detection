...
class Head_SSL(data.Dataset):
    def __init__(self, pathDataset, mode, size=[384, 384], be_consistency=False, patch_size=192, pre_crop=False,
                 rand_psize=False, ref_landmark=None, prob_map=None, retfunc=1, use_prob=True, ret_name=False, num_repeat=0):
...
class Testset_Head_SSL(data.Dataset):
    def __init__(self, pathDataset, mode, size=[384, 384], id_oneshot=126, pre_crop=False):
        ...
        if mode == 'Oneshot':
            print("Head One shot ID: ", id_oneshot)
            self.pth_Image = os. path.join(self.pth_Image, 'TrainingData')
            start = int(id_oneshot)
            end = int(id_oneshot)
        elif mode == 'Fewshots':
            self.pth_Image = os.path.join(self.pth_Image, 'TrainingData')
            start = 1
    ...
    def __getitem__(self, index):
        ...
        if self.mode not in ['Oneshot', 'Fewshots']:
            return item['ID'], item['image'], landmark_list
