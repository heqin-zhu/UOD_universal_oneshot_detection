...
class Hand(Dataset):
    def __init__(self, img_dir, gt_dir, img_size, num_landmark, sigma, alpha, use_actionmap=False, use_offset=False, img_size_highres=None, highres_factor=1, phase='train', cache_dir='.cache_hand', pseudo_path='', train_with_pseudo=False, few_shot=None, **kargs):
        # pseudo_label
        ...
        self.labels = pd.read_csv(os.path.join(gt_dir, 'all.csv'), header=None, index_col=0)
        if phase == 'train':
        ...
        self.norm_trans = transforms.Compose([
            transforms.Normalize([0.4087], [0.2028])
        ]
        )
    ...
    def __getitem__(self, idx):
        ...
        gt_origin = np.array([(gt_xys[i], gt_xys[i+1]) for i in range(0, len(gt_xys),2)])

        # print('-'*10, img_name)
        # print('origin', gt_origin[-5:])
        ...
        if self.train_with_existed_pseudo:
            ...
            with open(pseudo_file) as f:
                ...
                for n in range(self.num_landmark):
                    x, y = point_dic[str(n)]
                    gt_origin[n] = x/384*ori_size[1], y/384*ori_size[0]


        # print('pseudo', gt_origin)
        # exit()
        # stg1 size, low res
